...
if ($roll_no || $course_id > 0) {
    ...
    if ($roll_no) {
        ...
        foreach ($students_list as $student) {
            ...
            $count++;
            if ($count % 2 == 0)
                echo '</div>'; // Close wrapper after 2 marksheets
        }
        if ($count % 2 != 0)
            echo '</div>'; // Close last wrapper if odd number of students
        echo '</div>';
    }
    echo '</body></html>';
    exit;
...
function get_marksheet_css()
{
    ...
    <style>
        ...
        .marks-table th, .marks-table td { border: 1px solid #555; padding: 6px 4px; text-align: center; vertical-align: middle; }
...
function render_ug_marksheet($student)
{
    ...
    if (isset($student['qr_status']) && $student['qr_status'] == 1) {
        $html .= '<div style="margin-right:15px;"><img src="' . $qr_image_url . '" style="width:110px; height:110px; border:1px solid #ddd; padding:2px;"></div>';
    } else {
