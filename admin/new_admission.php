...
if (isset($_POST['submit_admission'])) {
    ...
    if ($err_msg == '') {
        ...
        // AUTO-GENERATE UIN for NEW ADMISSION
        if ($mode == 'new') {
            // Extract Year from Batch (e.g., 2025-26 -> 2025)
            $uin_year = date('Y');
            if (!empty($batch) && preg_match('/(\d{4})/', $batch, $matches)) {
                $uin_year = $matches[1];
            }

            // Check in Both tables (Register and Active Student Info) for the latest UIN for THIS BATCH YEAR
            $uin_last_res = $db->query("SELECT uin FROM (
                SELECT uin FROM uin_register_student WHERE uin LIKE 'RM" . $uin_year . "%'
                UNION
                SELECT uin FROM student_info WHERE uin LIKE 'RM" . $uin_year . "%'
            ) AS combined_uin ORDER BY uin DESC LIMIT 1");

            if ($uin_last_res && $uin_last_res->num_rows > 0) {
                $last_uin_obj = $uin_last_res->fetch_object();
                $last_uin_str = $last_uin_obj->uin;
                // Format: RM20250001. Logic: +1 to numeric part.
                $prefix = "RM";
                $num_part = str_replace($prefix, '', $last_uin_str);
                if (is_numeric($num_part)) {
                    $new_num = $num_part + 1;
                    $uin = $prefix . $new_num;
                } else {
                    // Fallback if weird format
                    $uin = $prefix . $uin_year . '0001';
                }
            } else {
                $uin = 'RM' . $uin_year . '0001';
            }
        }
        ...
        if ($mode == 'edit') {
            ...
            if (!empty($reason_ids)) {
                ...
                $sql_copy = "INSERT INTO student_info2 
                    (stu_name, father_name, mother_name, class, batch, dob, date_of_admission, gender, photo_id, signature_id, form_no, category, sub1, sub2, sub3, ot_sub1, ot_sub2, ot_sub3, status, income_certificate, acc_no, annual_income, other_univ, p_mobile, user_id, minority, physical_handicapped, fees, p_house, p_district, p_state, p_pin, email, aadhaar, whatsapp_mobile, uin, student_info_sno, reason, parent_mobile)
                    SELECT stu_name, father_name, mother_name, class, batch, dob, date_of_admission, gender, photo_id, signature_id, form_no, category, sub1, sub2, sub3, ot_sub1, ot_sub2, ot_sub3, status, income_certificate, acc_no, annual_income, other_univ, p_mobile, user_id, minority, physical_handicapped, fees, p_house, p_district, p_state, p_pin, email, aadhaar, whatsapp_mobile, uin, sno, '$reason_str', parent_mobile
            ...
            $sql_update = "UPDATE student_info SET 
                ...
                gender='$gender', photo_id='$photo_db_val', signature_id='$sig_db_val', form_no='$form_no', category='$category', 
            ...
            $insert_fields = "stu_name, father_name, mother_name, class, batch, dob, date_of_admission, 
                gender, form_no, category, sub1, sub2, sub3, ot_sub1, ot_sub2, ot_sub3, status, 
            ...
            $insert_values = "'$s_name', '$f_name', '$m_name', '$class_id', '$batch', '$dob', '$doa', 
                '$gender', '$form_no', '$category', '$sub1', '$sub2', '$sub3', '$ot_sub1', '$ot_sub2', '$ot_sub3', 2, 
...
<script>
    ...
    function populateDropdowns() {
        ...
        globalData.subjects.main.forEach(function (s) {
            mainOpts += '<option value="' + s.subject + '">' + s.subject + '</option>';
        });
    ...
    function fillFormFromUIN(d) {
        $('#uin_val').val(d.uin);
        $('#s_class').val(d.course_applied_for).trigger('change');
